...
Binput.addEventListener("focus", () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        span.addEventListener("click", async () => {
            if (Search.querySelectorAll(".searchone")) {
                let divs = Search.querySelectorAll(".searchone")
                for (let l of divs) {
                    l.style.display = 'none'
                }
            }
            let keyword = span.innerHTML;
            ...
            for (let k = 0; k < result.length; k++) {
                ...
                div2.innerHTML = result[k].title
                div3.innerHTML = result[k].artist.reduce((a, b) => {
                    return a + " " + b
                })
